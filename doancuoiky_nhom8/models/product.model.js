...
export default {
    ...
    async getProductsDisplayByCard(id_acc, productList) {
        ...
        for (let i = 0; i < productList.length; i++) {
            ...
            productList[i].real_time_start = moment(productList[i].time_start);
    ...
    async findExpriredProduct() {
        const proList = await db.raw(`SELECT p.id            as product_id,
                                             p.time_end      as expired_date,
                                             p.name          as product_name,
                                             p.price         as price,
                                             p.id_win_bidder as id_win_bidder,
                                             p.id_seller     as id_seller,
                                             a.name             seller_name,
                                             a.email         as seller_email
                                      FROM products p
                                               join accounts a on p.id_seller = a.id
    ...
    async ignoreBidder(product_info, bidder_info, domain) {
            ...
            if (auctionHistory.length == 1) {
                ...
                for (let i = 1; i < auctionHistory.length; i++) {
                    ...
                    if (beIgnored === null) {
                        ...
                        await this.deleteAuctionHistory({
                            id_acc: auctionHistory[i].bidder_id,
                            id_product: product_info.id,
        ...
        await biddingModel.deletePriceOfBidder({
            id_acc: bidder_info.id,
            id_product: product_info.id
        });
    ...
    async getWonProductByIdAcc(id_acc, limit, offset) {
        const sql = `SELECT *
                     from products p
                     where p.time_end < now()
                       and p.id_win_bidder = ${id_acc} limit ${limit}
                     offset ${offset}`;
        const raw = await db.raw(sql);
        return raw[0];
    },
    async countWonProductByIdAcc(id_acc) {
        const sql = `SELECT COUNT(id) as amount
                     from products p
                     where p.time_end < now()
                       and p.id_win_bidder = ${id_acc}`;
        const raw = await db.raw(sql);
